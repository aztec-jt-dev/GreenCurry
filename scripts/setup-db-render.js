...
if (!connectionString) {
    ...
    console.log('   2. Run: DATABASE_URL="your_url" node scripts/setup-db-render.js');
    process.exit(1);
}
...
async function setup() {
    ...
    try {
        ...
        await client.query('DROP TABLE IF EXISTS greencurry_bookings CASCADE');
        await client.query('DROP TABLE IF EXISTS greencurry_rooms CASCADE');
        await client.query('DROP TABLE IF EXISTS greencurry_users CASCADE');
    ...
    `, ['admin', 'curry123#', 'admin']);
      ...
      `, [room.id, room.name, room.type, room.basePrice, room.hasToilet, room.capacity]);
        ...
        console.log('\n🚀 Your Green Curry Hostel database is ready!');
